...
"""################################ Beam Path Euler 2 ########################################"""
r_0_h2 = np.ones(len(r_2))*r_0
alpha_euler_2 = []
for i in range(len(phi_0_rad)):
    r_h2 = np.sqrt(pow(r_2, 2) + pow(r_0_h2, 2) - 2 * np.multiply(r_2, np.multiply(r_0_h2, np.cos(theta_euler_2[:,i]))))
    gamma_euler_2 = np.arcsin(np.multiply(np.divide(r_0_h, r_h2), np.sin(theta_euler_2[:,i])))
    alpha_euler_2.append(gamma_euler_2 + theta_euler_1[:,i])
    alpha_euler_2[-1][0] = phi_0_rad[i]
alpha_euler_deg_2 = np.rad2deg(alpha_euler_2)

################### Beam Path RK4 2 ########################################
alpha_RK4_2 = []
for i in range(len(phi_0_rad)):
    r_h2 = np.sqrt(pow(r_2, 2) + pow(r_0_h2, 2) - 2 * np.multiply(r_2, np.multiply(r_0_h2, np.cos(theta_RK4_2[:,i]))))
    gamma_RK4_2 = np.arcsin(np.multiply(np.divide(r_0_h2, r_h2), np.sin(theta_RK4_2[:,i])))
    alpha_RK4_2.append(gamma_RK4_2 + theta_RK4_2[:,i])
    alpha_RK4_2[-1][0] = phi_0_rad[i]
alpha_deg_2 = np.rad2deg(alpha_RK4_2)
